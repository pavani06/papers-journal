...
- arquitetura: markdown com frontmatter padronizado, wikilinks e MOCs; `obsidian-eval` para consulta sem app; `papers/` é symlink para `/home/pavanpavan/papers-journal/edicoes` — mesmo arquivo físico, não cópia (constraint: `facts/_global/constraints.md:67`); commits de sessão (`sessions/`, `state/`, `oracle-reviews/`) são rotina esperada e tendem a classificar drift `externo` inofensivo. A evolução das regras globais acontece aqui: `facts/_global/principles.md` acumula princípios com evidência por contagem agregada ("evidence: 2 handoffs", "evidence: 21 papers + 1 par de transferência", `facts/_global/principles.md:16-22`), e a promoção automática é por recorrência textual com limiar de 3 ocorrências, sem estratificação nem holdout (`facts/_global/promote-patterns-validation.md:27-39`, modo fuzzy em `:58-64`). Adoções recentes vieram do catálogo de princípios do papers-synth sob aprovação nomeada do operador. Refresh de 2026-09-02 (subagente papers-deep, confrontos do snapshot 2026-09-01): 341 arquivos/+12.913 linhas desde `0b6a0dd7` — epic #53 (catálogo de 274 handles em `catalog/harness-sessions/`, indexador read-only com cursor, protocolo de Promoção de outcome no `index.md`, cópia canônica do AGENTS.md global em `policy/agents-global.md` com checker de drift), CL-EPIC 1 (loop-ledger append-only em `state/`, evidence gate para fatos principle), federação de 7 vaults; claims de `facts/_global/` e `index.md` re-verificadas por leitura direta.
- last-verified: 2026-09-02
- verified-head: 10b176c927d6d63e368ccf43773f4b87ab03bdec
...
- arquitetura: markdown Obsidian com frontmatter e wikilinks validados (`npx tsx scripts/validate-obsidian.ts`, gate de CI); `docs/canonical/` com ~180 padrões, `docs/analysis/` gerado pelo pipeline analyze-and-improve (analysis, mental-model, patterns, classification por fonte), `docs/system-of-record.md` com precedência documental e os domínios que governam a taxonomia de tags, `docs/cross-vault-taxonomy.md` mapeando equivalências com os outros vaults, `curriculum/` (níveis 1-4 + core concepts + case studies), `harness/` (guia e templates do analyze-and-improve), portal web estático em `web/`; sem código de runtime além dos scripts de validação frontmatter. Refresh de 2026-09-02 (subagente papers-deep, confronto do 2608.29464): 2 commits desde `3e6e124d` (ADR de vault federation em `docs/decisions/`; migração `@pavani/obsidian-eval` → `@pavani_org/obsidian-eval` ^0.3.0); `docs/canonical/3-layer-evaluation-architecture.md` e `docs/system-of-record.md` re-verificados por leitura direta nos pontos citados.
- last-verified: 2026-09-02
- verified-head: 48c5d45b020ced2aab17c94d6a89594ca63829c6
